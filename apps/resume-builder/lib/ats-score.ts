...
export function hasMeasurableImpact(data: ResumeData): boolean {
  const numberPattern = /[\d%]|\d+[kK]|[\d.]+\s*x\b/i;
  const check = (s: string) => numberPattern.test(s);
  for (const e of data.experience) {
    if (e.details && check(e.details)) return true;
  }
  for (const p of data.projects) {
    if (p.description && check(p.description)) return true;
  }
  return false;
}

function summaryContainsActionVerbs(summary: string): boolean {
